...
- **A release's assets are now uploaded one at a time, with retries, to a draft release that is verified before it is published.** `softprops/action-gh-release@v2` uploads every asset concurrently, has no per-file retry, and publishes the release before anything is checked: on the MariaDB wave that step failed or stalled on 5 of 8 attempts (`Error saving asset`, `Headers Timeout Error`, `Error creating asset temp dir`, and a 46-minute stall on the 450 MB `linux-x64` archive that a hand-run `gh release upload` then finished in 44 seconds). All 22 `release-<engine>.yml` workflows now call `builds/common/publish-release.sh`, which creates the release as a **draft**, uploads each asset sequentially with `gh release upload --clobber` (3 attempts, 5s then 15s backoff) and `checksums.txt` last, verifies that every asset is attached, in state `uploaded` and the right size (comparing the API's `digest` field against `checksums.txt` where it is exposed), and only then publishes. Every pre-upload validation step is unchanged, the job still needs nothing beyond `contents: write`, and this retires the last `softprops/action-gh-release@v2` usage in the repo.
- **A re-dispatch against an already-published release can no longer leave its public assets half-replaced.** A published release cannot be staged behind a draft, because demoting it would retract a tag consumers already resolve, so the replacement assets used to be clobbered straight onto the live names, `checksums.txt` last, with no rollback: a failure partway through a partial-platform rebuild left the release carrying a mix of new and old archives plus a stale `checksums.txt` until a repair run. `publish-release.sh` now **stages the swap**. Every replacement is uploaded under a throwaway `<name>.staging-<run id>` name, the whole set is verified there (attached, state `uploaded`, size, and `digest` against `checksums.txt` where the API exposes it), and only then is each asset renamed onto its public name through `PATCH /repos/{owner}/{repo}/releases/assets/{asset_id}`, `checksums.txt` last. `--clobber` never targets a live name, so no unverified byte ever carries one, and the mixed-state window shrinks from the length of an upload to one quick API call per asset. Any failure before the swap deletes the staging assets and leaves the public asset set exactly as it was; an unavailable rename endpoint refuses outright rather than falling back to clobber; a swap that fails part-way prints exactly which assets were swapped and which were not, and a re-run finishes it. The release is still never demoted back to a draft, and the `checksums.txt` swapped in is the merged file from `merge-release-checksums.sh`, so it describes the full public set. Covered by `tests/release-publish-staged-swap.test.ts`.
...
- **`retired_platforms`, for a platform that was released and later dropped.** Removing a platform from a version's `platforms` list never removes its release: R2 URLs are immutable and `releases.json` is rebuilt from the GitHub releases, so the entry outlives the support decision and `pnpm prep` reported it as an orphaned release on every run. A version can now name the dropped platform with the reason it was dropped, which is what tells prep the leftover entry is expected. The platform stays absent from `platforms`, so nothing about what consumers are offered changes, and the resolver is untouched. prep also validates the declarations: a retirement whose release no longer exists, or one that contradicts `platforms`, is reported in its own right, so the list cannot become a place to silence warnings. `tests/retired-platforms.test.ts` enforces the same rules plus the stronger one prep only warns about, that every released platform is either supported or explicitly retired. Schema updated.
- **Two standing win32-x64 orphans are now declared, not ignored.** `ferretdb` 2.7.0 and `postgresql-documentdb` 17-0.107.0 both released a `win32-x64` artifact on 2026-01-23 and both lost Windows support on 2026-02-15 (`ac9eca8`), leaving two warnings on every `pnpm prep` since. The drop was correct and stands: FerretDB v2 runs on postgresql-documentdb, and that engine's Windows bundle carries pgvector only, because the documentdb extension (plus pg_cron, rum, PostGIS) has no Windows build, so a Windows FerretDB v2 has nothing to run against. Both artifacts are still on R2 and still resolve (HTTP 200, sizes matching `releases.json`), so neither release entry can be pruned; both are now recorded as retired with that history. Windows FerretDB remains v1 (1.24.2), which is unaffected and stays on the engine's platform list. `pnpm prep` runs with zero warnings again.
