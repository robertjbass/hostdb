#!/usr/bin/env tsx
/**
 * Download official Qdrant binaries for re-hosting
 *
 * Usage:
 ...
 *
 * Qdrant distributes:
 * - Linux/macOS: tar.gz archives containing single binary
 * - Windows: zip archive containing single binary
 */
...
function repackage(
  ...
  const qdrantDir = resolve(tempDir, 'qdrant')

  mkdirSync(qdrantDir, { recursive: true })

  // Copy binary to package directory
  const binaryName = platform.startsWith('win32') ? 'qdrant.exe' : 'qdrant'
  const destBinary = resolve(qdrantDir, binaryName)
  ...
  writeFileSync(
    resolve(qdrantDir, '.hostdb-metadata.json'),
  ...
  if (platform.startsWith('win32')) {
    execFileSync('zip', ['-rq', outputPath, 'qdrant'], {
      stdio: 'inherit',
      cwd: tempDir,
    })
  } else {
    execFileSync('tar', ['-czf', outputPath, '-C', tempDir, 'qdrant'], {
      stdio: 'inherit',
    })
  }
    ...
    switch (args[i]) {
      ...
      case '--version': {
        ...
        if (!isValidVersion(versionValue)) {
          ...
          logError('Version must be in format: X.Y.Z (e.g., 1.16.3)')
          process.exit(1)
          break
        }
      ...
      case '-h':
        console.log(`
Usage: pnpm download:qdrant [options]

Options:
  --version VERSION    Qdrant version (default: 1.16.3)
...
async function main() {
  ...
  console.log()
  logInfo(`Qdrant Download Script`)
  ...
  for (const platform of platforms) {
    ...
    const downloadExt = source.format
