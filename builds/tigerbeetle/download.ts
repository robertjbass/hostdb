#!/usr/bin/env tsx
/**
 * Download official TigerBeetle binaries for re-hosting
 *
 * Usage:
 ...
 *
 * TigerBeetle distributes zip archives for all platforms,
 * each containing a single binary.
 */
...
function repackage(
  ...
  const tbDir = resolve(tempDir, 'tigerbeetle')
  ...
  const binaryName = platform.startsWith('win32')
    ? 'tigerbeetle.exe'
    : 'tigerbeetle'
  const destBinary = resolve(tbDir, binaryName)
  ...
  writeFileSync(
    resolve(tbDir, '.hostdb-metadata.json'),
  ...
  if (platform.startsWith('win32')) {
    execFileSync('zip', ['-rq', outputPath, 'tigerbeetle'], {
      stdio: 'inherit',
      cwd: tempDir,
    })
  } else {
    execFileSync('tar', ['-czf', outputPath, '-C', tempDir, 'tigerbeetle'], {
      stdio: 'inherit',
    })
  }
...
function parseArgs(): {
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '-h':
        console.log(`
Usage: pnpm download:tigerbeetle [options]

Options:
  --version VERSION    TigerBeetle version (default: 0.16.70)
...
async function main() {
  ...
  console.log()
  logInfo(`TigerBeetle Download Script`)
  ...
  for (const platform of platforms) {
    ...
    const downloadPath = resolve(
      outputDir,
      'downloads',
      `tigerbeetle-${version}-${platform}-original.zip`,
    )
    const outputPath = resolve(
      outputDir,
      `tigerbeetle-${version}-${platform}.${ext}`,
    )
    ...
    // Extract binary (all platforms are zip)
