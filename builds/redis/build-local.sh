#!/usr/bin/env bash
#
# Local Docker build script for Redis
#
# Usage:
#   ./builds/redis/build-local.sh [options]
#
# Options:
#   --version VERSION    Redis version to build (default: 8.4.0)
...
#
# Examples:
#   ./builds/redis/build-local.sh
#   ./builds/redis/build-local.sh --version 8.4.0 --platform linux-arm64
#   ./builds/redis/build-local.sh --no-cache --cleanup
#
...
OUTPUT_PATH="${OUTPUT_DIR}/redis-${VERSION}-${PLATFORM}"
mkdir -p "$OUTPUT_PATH"

log_info "Building Redis ${VERSION} for ${PLATFORM}"
...
# Verify output
if [[ -f "${OUTPUT_PATH}/redis/bin/redis-server" ]]; then
    log_success "redis-server binary found"
else
    log_error "redis-server binary NOT found - build may have failed"
    exit 1
fi

if [[ -f "${OUTPUT_PATH}/redis/bin/redis-cli" ]]; then
    log_success "redis-cli binary found"
else
    log_warn "redis-cli binary not found"
fi

# Create tarball
TARBALL="${OUTPUT_DIR}/redis-${VERSION}-${PLATFORM}.tar.gz"
log_info "Creating tarball: ${TARBALL}"

tar -czvf "${TARBALL}" -C "${OUTPUT_PATH}" redis
...
echo ""
log_success "Redis ${VERSION} for ${PLATFORM} built successfully!"
