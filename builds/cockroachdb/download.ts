#!/usr/bin/env tsx
/**
 * Download official CockroachDB binaries for re-hosting
 *
 * Usage:
 ...
 *
 * CockroachDB distributes:
 * - Linux/macOS: tgz archives containing cockroach binary and libs
 * - Windows: zip archive containing cockroach.exe
 */
...
function repackage(
  ...
  const cockroachdbDir = resolve(tempDir, 'cockroachdb')

  mkdirSync(cockroachdbDir, { recursive: true })

  // Copy binary to package directory
  const binaryName = platform.startsWith('win32')
    ? 'cockroach.exe'
    : 'cockroach'
  const destBinary = resolve(cockroachdbDir, binaryName)
  ...
  writeFileSync(
    resolve(cockroachdbDir, '.hostdb-metadata.json'),
  ...
  if (platform.startsWith('win32')) {
    execFileSync('zip', ['-rq', outputPath, 'cockroachdb'], {
      stdio: 'inherit',
      cwd: tempDir,
    })
  } else {
    execFileSync('tar', ['-czf', outputPath, '-C', tempDir, 'cockroachdb'], {
      stdio: 'inherit',
    })
  }
    ...
    switch (args[i]) {
      ...
      case '--version': {
        ...
        if (!isValidVersion(versionValue)) {
          ...
          logError('Version must be in format: X.Y.Z (e.g., 25.4.2)')
          process.exit(1)
          break
        }
      ...
      case '-h':
        console.log(`
Usage: pnpm download:cockroachdb [options]

Options:
  --version VERSION    CockroachDB version (default: 25.4.2)
...
async function main() {
  ...
  console.log()
  logInfo(`CockroachDB Download Script`)
  ...
  for (const platform of platforms) {
    ...
    const downloadExt = source.format === 'zip' ? 'zip' : 'tgz'
    const downloadPath = resolve(
      outputDir,
      'downloads',
      `cockroachdb-${version}-${platform}-original.${downloadExt}`,
    )
    const outputPath = resolve(
      outputDir,
      `cockroachdb-${version}-${platform}.${ext}`,
    )
