#!/usr/bin/env tsx
/**
 * Download FerretDB v1.x binaries and bundle with MongoDB tools
 *
 * This script:
 * - Downloads official FerretDB v1.x binaries for Linux
 * - Cross-compiles FerretDB for macOS/Windows using Go
 * - Bundles mongosh and database-tools for complete MongoDB compatibility
 *
 * FerretDB v1.x uses plain PostgreSQL as its backend (no DocumentDB extension).
 *
 * Usage:
 *   ./builds/ferretdb-v1/download.ts [options]
 *   pnpm tsx builds/ferretdb-v1/download.ts [options]
 *
 * Options:
 *   --version VERSION    FerretDB version (default: 1.24.2)
...
// Validate version format (e.g., "1.24.2")
...
function crossCompileFerretDB(
  ...
  const repoDir = join(outputDir, 'ferretdb-v1-source')
  const binaryName = platform.startsWith('win32') ? 'ferretdb.exe' : 'ferretdb'
  const outputPath = join(
    outputDir,
    `ferretdb-v1-${version}-${platform}`,
    binaryName,
  )
...
async function repackage(
  ...
  try {
    ...
    if (isDownloadableSource(source)) {
      // Download raw binary
      const binaryPath = join(downloadDir, `ferretdb-v1-${version}-${platform}`)
    ...
    switch (args[i]) {
      ...
      case '--version': {
        ...
        if (!isValidVersion(versionValue)) {
          ...
          logError('Version must be in format: X.Y.Z (e.g., 1.24.2)')
          process.exit(1)
        }
      ...
      case '-h':
        console.log(`
Usage: ./builds/ferretdb-v1/download.ts [options]

Downloads and bundles FerretDB v1.x with MongoDB tools:
...
Examples:
  ...
  ./builds/ferretdb-v1/download.ts --version 1.24.2 --platform linux-x64
  ./builds/ferretdb-v1/download.ts --all-platforms
`)
        process.exit(0)
        break
    }
...
async function main() {
  ...
  console.log()
  logInfo(`FerretDB v1 Download Script (with bundled MongoDB tools)`)
  logInfo(`FerretDB Version: ${version}`)
  logInfo(`Backend: Plain PostgreSQL (no DocumentDB)`)
  ...
  for (const platform of platforms) {
    ...
    const outputPath = resolve(
      outputDir,
      `ferretdb-v1-${version}-${platform}.${ext}`,
    )
