#!/usr/bin/env bash
#
# Local Docker build script for MariaDB
#
# Usage:
#   ./builds/mariadb/build-local.sh [options]
#
# Options:
#   --version VERSION    MariaDB version to build (default: 11.4.5)
...
#
# Examples:
#   ./builds/mariadb/build-local.sh
#   ./builds/mariadb/build-local.sh --version 11.8.5 --platform linux-arm64
#   ./builds/mariadb/build-local.sh --no-cache --cleanup
#
...
case $PLATFORM in
    ...
    *)
        ...
        log_error "For darwin platforms, use official binaries or MariaDB4j"
        exit 1
        ;;
esac
...
OUTPUT_PATH="${OUTPUT_DIR}/mariadb-${VERSION}-${PLATFORM}"
mkdir -p "$OUTPUT_PATH"

log_info "Building MariaDB ${VERSION} for ${PLATFORM}"
...
# Run Docker build
log_info "Starting Docker build (this may take 45-90+ minutes)..."
echo ""
...
# Verify output
if [[ -f "${OUTPUT_PATH}/mariadb/bin/mariadbd" ]]; then
    log_success "mariadbd binary found"
else
    log_error "mariadbd binary NOT found - build may have failed"
    exit 1
fi

if [[ -f "${OUTPUT_PATH}/mariadb/bin/mariadb" ]]; then
    log_success "mariadb client binary found"
else
    log_warn "mariadb client binary not found"
fi

# Create tarball
TARBALL="${OUTPUT_DIR}/mariadb-${VERSION}-${PLATFORM}.tar.gz"
log_info "Creating tarball: ${TARBALL}"

tar -czvf "${TARBALL}" -C "${OUTPUT_PATH}" mariadb
...
echo ""
log_success "MariaDB ${VERSION} for ${PLATFORM} built successfully!"
